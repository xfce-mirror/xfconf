...
#define EXPORT_FORMAT_VERSION "0.1"
#define EXPORT_FILE_HEADER "Generated by xfconf-query --export, do not modify this line; format version: " EXPORT_FORMAT_VERSION
...
xfconf_query_export(void)
{
    ...
    if (contents != NULL) {
        gchar *temp = g_strconcat(EXPORT_FILE_HEADER, "\n", contents, NULL);
        g_free(contents);
        contents = temp;
...
xfconf_query_import_check(const gchar *contents)
{
    ...
    if (g_strcmp0(lines[0], EXPORT_FILE_HEADER) != 0) {
        xfconf_query_printerr(_("Aborted on line 1: corrupted version header"));
        g_strfreev(lines);
        return NULL;
    }
